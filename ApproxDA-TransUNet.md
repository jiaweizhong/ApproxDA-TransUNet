...
| ISIC 2018 | Skin lesion (binary) | Low CS (**0.50pp**) | gate=learn, M=7, r=32 | **+1.15%** (vs our re-run 88.43%; +0.70% vs paper 88.88%) |
...
| ISIC | 88.43% (ours, 300ep SGD) | **89.58%** (gate=learn, M=7, r=32) | **+1.15%** (vs ours) | **0.50pp** (gate=pam ablation ✅) | Low CS |
...
**Figure 7 — Window Sensitivity (Phase D, CS proxy) ✅ Done (all datasets)**
DSC vs window size M for Synapse (4-point: M=7/28/56/112) and Kvasir (4-point: M=7/28/56/112).
Synapse: M=28 peak 80.94%, range **2.30pp** (high CS). Kvasir: M=56 peak 90.17%, range **0.64pp** (near-flat, low CS).
Replaces Phase C (center-crop sensitivity) which was INVALID — full-label evaluation artifact on both datasets.
Journal version will extend to 5-dataset curves (ACDC 0.73pp, CVC 0.62pp, ISIC 0.50pp all confirmed).
