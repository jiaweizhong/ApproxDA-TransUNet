...
for i in range(4):
    arrow(
        ax,
        XS[i] + (BW_IO if i == 0 else BW_ENC) / 2,
        Y_ENC,
        XS[i + 1] - (BW_VIT if i == 3 else BW_ENC) / 2,
        Y_ENC,
    )
...
for i in [3, 2, 1]:
    arrow(
        ...
        XS[i] - BW_DEC / 2,
        Y_DEC,
        XS[i - 1] + (BW_IO if i == 1 else BW_DEC) / 2,
        Y_DEC,
    )
...
box(ax, VIXC, vit_xi_y, VBW, h_xi, C_IO, "Input tokens", "196 × 768", fs=11, sfs=9.5)
...
plt.savefig(
    os.path.join(out_dir, "approxda_overview.pdf"), bbox_inches="tight", dpi=300
)
plt.savefig(
    os.path.join(out_dir, "approxda_overview.png"), bbox_inches="tight", dpi=200
)
