...
Following DA-TransUNet, we train with SGD (momentum 0.9, weight decay $10^{-4}$) with initial learning rate 0.01 and polynomial decay schedule. Total batch size is 24 and input images are resized to $224 \times 224$ with random horizontal flipping and random rotation for data augmentation. Training runs for 300 epochs. All experiments use PyTorch 2.8.0 (CUDA 12.8, cuDNN 9.10) with NVIDIA driver 580.159.03. The DA-TransUNet baseline is trained on a single NVIDIA Tesla T4 (15 GB VRAM). AdaDA-TransUNet multi-GPU experiments use 2$\times$NVIDIA L4 (24 GB VRAM each) with \texttt{CUDA\_VISIBLE\_DEVICES=0,1}, per-GPU batch size 12 (total effective batch size 24, identical to the single-GPU baseline), and base learning rate 0.01 unchanged, providing a controlled like-for-like comparison. We also validate DataParallel correctness on 2$\times$T4 using the same configuration. We report wall-clock training time and peak VRAM to compare hardware efficiency alongside segmentation accuracy.

To validate our experimental setup, we reproduced DA-TransUNet under identical conditions (same hardware, optimizer, and random seed). Our reproduction achieves 79.28\% DSC and 24.48 mm HD95. The DA-TransUNet paper reports a best result of 79.80\% DSC~\cite{sun2024datransunet}, so our reproduction is 0.52\% below the published figure. This gap is attributable to training duration: the original paper trains for 500 epochs on an RTX 3090, while our setup uses 300 epochs. Both our AdaDA and reproduced DA-TransUNet baselines use the final-epoch checkpoint for a controlled, fair comparison.
...
This finding provides a natural explanation for the gap between the paper's reported 79.80\% and our reproduction at 79.36\%: the paper's own ablation (Table 5 in~\cite{sun2024datransunet}) shows that applying DA only to the first skip connection achieves 79.36\% DSC, closely matching our reproduction at 79.28\% (the small residual gap is expected given we train for 150 epochs vs.\ the paper's 500). Full three-skip coverage yields 79.80\%. Our reproduction faithfully replicates the published code's two-skip behavior.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    ...
    DA-TransUNet$^\dagger$ (reproduced)       & 79.28 & 24.48 \\
    \midrule
  ...
  \end{center}
  \vspace{-0.4em}
  \footnotesize{Baselines from published papers. DA-TransUNet† denotes our reproduction (79.28\% vs.\ published 79.80\%); the 0.52\% gap is attributable to training duration (150 vs.\ 500 epochs)---both our models use the final-epoch checkpoint for a fair comparison. $^*$Preliminary result from a partial training run (\textasciitilde{}130 epochs); full 300-epoch result pending.}
\end{table}
...
\begin{table*}[t]
  \caption{Efficiency Comparison on Synapse (batch=24 total, 300 epochs). DA-TransUNet on T4$\times$1; AdaDA on L4$\times$2 (batch=12/GPU, total=24, lr=0.01 unchanged).}
  ...
  \begin{tabular}{lccccccc}
    ...
    DA-TransUNet~\cite{sun2024datransunet} & 107.95      & 25.5 & 5.6h (T4$\times$1) & 11.5 GB   & 121.7            & 0.5 GB           & No$^\ddagger$ \\
    \midrule
    \multirow{2}{*}{\textbf{AdaDA ($r$=32, $M$=7, $G$=8)}}
      ...
      & & & \textit{XX}h (L4$\times$2)$^\S$ & \textit{XX} & & & Yes \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{-0.4em}
...
\begin{table*}[t]
  \caption{Ablation Study on Synapse Multi-Organ CT (L4$\times$2, SGD, lr=0.01, batch=12/GPU (total=24), 300 epochs, seed=1234)}
  \label{tab:ablation}
  \begin{center}
  \setlength{\tabcolsep}{5pt}
  \begin{tabular}{lcccccc}
    ...
    \midrule
    DA-TransUNet (baseline)    & 79.28        & 24.48        & 25.5 & 11.5 GB & 121.7        & 0.5 GB       \\
