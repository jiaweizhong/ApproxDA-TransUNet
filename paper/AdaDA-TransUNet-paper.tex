...
\begin{table*}[t]
  ...
  $^\S$~AdaDA multi-GPU experiments use 2$\times$T4 via \texttt{torchrun} (DistributedDataParallel, NCCL backend), per-GPU batch size 12 (total=24, matching the single-GPU baseline) and base LR 0.01 unchanged—identical hardware to the baseline for a like-for-like comparison. AdaDA's windowed low-rank PAM reduces the attention matrix from $(B\!\times\!12544^2)$ to $(B\!\times\!256\!\times\!49\!\times\!32)$ at the $112\!\times\!112$ skip connection—a ${\sim}390\times$ reduction (windowing alone: ${\sim}260\times$)—enabling multi-GPU DDP training and approximately halving wall-clock time.}
\end{table*}

The efficiency advantage of AdaDA-TransUNet is primarily \emph{memory}, not compute throughput. The backbone (ResNet encoder + 12-layer ViT) accounts for the majority of FLOPs and is shared between both models; the dual-attention blocks contribute only ${\sim}21\%$ of total FLOPs but dominate memory. Replacing global $\mathcal{O}(N^2)$ PAM with windowed low-rank attention reduces the attention matrix at the $112\!\times\!112$ skip connection from $(B\!\times\!12544\!\times\!12544)$ to $(B\!\times\!256\!\times\!49\!\times\!49)$ — a ${\sim}260\times$ reduction — while the per-slice inference time decreases by ${\sim}15\%$ (121.8$\to$103.7\,s/vol). Crucially, the original DA-TransUNet code cannot utilize multi-GPU training regardless of skip configuration: the zero-element \texttt{query\_conv} weight in \texttt{DANetHead(64,64)} exists in the parameter set whether or not the 64-channel skip is active, causing \texttt{DataParallel} gradient synchronization to fail at the first \texttt{loss.backward()} (confirmed empirically on T4$\times$2). AdaDA-TransUNet runs cleanly on T4$\times$2 via DDP (\texttt{torchrun}), approximately halving wall-clock training time.
...
\]
The combined ${\sim}390{\times}$ reduction brings attention storage from 15\,GB to 38\,MB. AdaDA-TransUNet multi-GPU training uses DistributedDataParallel (DDP) via \texttt{torchrun} with NCCL all-reduce, which operates on per-process isolated CUDA contexts and does not encounter the \texttt{BroadcastBackward} gradient-synchronization failure that blocks \texttt{DataParallel}.
