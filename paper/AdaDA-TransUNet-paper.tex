...
\begin{enumerate}
  \item \textbf{Architectural incompatibility.} \texttt{DANetHead} compresses channels by $\lfloor C/16 \rfloor$ before constructing the PAM. For $C{=}64$: $\lfloor 64/16 \rfloor = 4$ intermediate channels. The PAM then creates \texttt{query\_conv} $=$ \texttt{Conv2d(4,\,$\lfloor 4/8 \rfloor$,\,1)} $=$ \texttt{Conv2d(4,\,0,\,1)}---a convolution with \emph{zero output channels}, which is architecturally invalid. PyTorch raises \texttt{RuntimeError: weight of size [0,\,4,\,1,\,1]} at the first forward pass, regardless of available memory. This failure was empirically confirmed on T4$\times$2 with the operator-precedence bug corrected.
  \item \textbf{Memory barrier.} Even if the channel arithmetic were patched, applying $N{\times}N$ PAM at $112{\times}112$ yields $N{=}12{,}544$ and an attention matrix of $B{\times}N{\times}N$ float32 values. At batch size 24 per GPU this requires ${\approx}15$\,GB---matching the full VRAM of a T4---exhausting device memory during the backward pass.
\end{enumerate}
...
\begin{table*}[t]
  \caption{Efficiency Comparison on Synapse (batch=24 total, 300 epochs). DA-TransUNet on T4$\times$1; AdaDA on T4$\times$2 (batch=12/GPU, total=24, lr=0.01 unchanged).}
  ...
  \begin{tabular}{lccccccc}
    ...
    DA-TransUNet~\cite{sun2024datransunet} & 107.95      & 25.5 & 22.35h (T4$\times$1) & 11.5 GB   & 121.8            & 0.5 GB           & No$^\ddagger$ \\
    \midrule
  ...
  $^\ddagger$~DA-TransUNet OOMs on 2$\times$T4: the $\mathcal{O}(N^2)$ PAM backward must store a $(12544\!\times\!12544)$ attention matrix per sample at the $112\!\times\!112$ skip connection, exhausting GPU memory during gradient aggregation.\\
...
\]
\texttt{Conv2d(4,\;0,\;1)} has weight shape \texttt{[0,\,4,\,1,\,1]}, which PyTorch rejects with:
\begin{verbatim}
RuntimeError: expected weight to be at least 1 at dimension 0,
              but got weight of size [0, 4, 1, 1]
\end{verbatim}
This error fires at the first forward pass---before any gradient is computed---regardless of batch size or GPU memory. The combined compression $\lfloor \cdot/16 \rfloor \circ \lfloor \cdot/8 \rfloor$ is safe only when $C \geq 128$ (giving $\lfloor 128/16\rfloor=8,\;\lfloor 8/8\rfloor=1$).
...
\]
At batch size $B{=}24$ per GPU:
\[
  24 \times 12{,}544^2 \times 4\;\text{bytes} \approx 15.0\;\text{GB},
\]
matching the full VRAM of a T4 and leaving no capacity for activations, optimizer states, or inter-GPU gradient aggregation in the backward pass.
...
\textbf{Memory.} Window partitioning and low-rank projection jointly reduce attention storage at the $112{\times}112$ level:
\begin{align*}
  \text{Full PAM:} &\quad B \!\times\! N^2 = B \!\times\! 12544^2 \approx 15.0\;\text{GB at }B{=}24 \\
  \text{Windowed only:} &\quad B \!\times\! n_W \!\times\! N_w^2 = B \!\times\! 256 \!\times\! 49^2 \approx 58\;\text{MB at }B{=}24 \quad({\sim}260{\times}) \\
  \text{Windowed + rank:} &\quad B \!\times\! n_W \!\times\! N_w \!\times\! r = B \!\times\! 256 \!\times\! 49 \!\times\! 32 \approx 38\;\text{MB at }B{=}24 \quad({\sim}390{\times})
\end{align*}
where $n_W{=}(112/7)^2{=}256$ windows of size $N_w{=}M^2{=}49$ tokens and $r{=}32$ is the projection rank. The combined ${\sim}390{\times}$ reduction brings attention storage from 15\,GB to 38\,MB, enabling multi-GPU DataParallel training on T4$\times$2 without modification.
