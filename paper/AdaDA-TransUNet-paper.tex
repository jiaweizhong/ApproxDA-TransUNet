...
\begin{table*}[t]
  ...
  $^\S$~AdaDA's windowed low-rank PAM reduces the attention matrix to $(256\!\times\!49\!\times\!49)$ at the same resolution — a ${\sim}260\times$ reduction — enabling 2-GPU DataParallel training and approximately halving wall-clock time. Note: \texttt{DataParallel} scatter produces non-contiguous tensor views; the implementation explicitly calls \texttt{.contiguous()} at skip-connection boundaries before windowed operations.}
\end{table*}
