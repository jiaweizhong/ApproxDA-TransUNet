...
To validate our experimental setup, we reproduced DA-TransUNet under identical conditions (same hardware, optimizer, and random seed). Our 300-epoch reproduction achieves \textbf{80.51\%} DSC and 25.41 mm HD95 using best-model checkpoint selection (checkpoint saved whenever validation DSC improves). This exceeds the DA-TransUNet paper's published 79.80\%~\cite{sun2024datransunet}; the gain is attributable to checkpoint selection policy---best-model selection across 300 epochs captures transient validation peaks that final-epoch evaluation can miss when the validation curve oscillates. Both our reproduced DA-TransUNet baseline and AdaDA-TransUNet use best-model checkpoint selection consistently for a fair comparison.
...
This behavior is not a coding oversight but a memory-driven constraint. Applying \texttt{DANetHead(64,64)} to the $112{\times}112$ skip would allocate a PAM spatial attention matrix of shape $(N \times N)$ with $N = 112^2 = 12{,}544$. At float32 with batch size 24, this matrix alone requires approximately 15 GB, exceeding the 15 GB VRAM of a single T4. The code silently bypasses this level through the mismatched condition, achieving the memory safety at the cost of reduced skip-connection coverage.

This finding is consistent with the paper's own ablation (Table 5 in~\cite{sun2024datransunet}), which shows single-skip DA coverage achieves 79.36\% and full three-skip coverage 79.80\%. Our reproduction, running the same two-skip effective code for 300 epochs with best-model selection, achieves 80.51\%---exceeding the paper's published figure, which we attribute to checkpoint selection policy rather than a fundamental performance difference. Our reproduction faithfully replicates the published code's two-skip behavior.

AdaDA-TransUNet's windowed low-rank PAM resolves this memory barrier. By partitioning the $112{\times}112$ feature map into $(112/7)^2 = 256$ non-overlapping $7{\times}7$ windows, each window's attention matrix is only $49{\times}49$, reducing total attention storage from $(B \times 12544 \times 12544)$ to $(B \times 256 \times 49 \times 49)$---a ${\sim}260{\times}$ reduction---well within T4 VRAM. This enables AdaDA to apply attention at all three skip levels.
...
\begin{enumerate}
  ...
  \item \textbf{AdaDA-3skip}: AdaDA blocks applied to all three skip connections including the $112{\times}112$ level, matching the paper's stated design intent. Only AdaDA can run this configuration; DA-TransUNet cannot without OOM.
\end{enumerate}
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    ...
    DA-TransUNet$^\dagger$ (reproduced)       & 80.51 & 25.41 \\
    \midrule
    \textbf{AdaDA-TransUNet (ours)}           & \textbf{78.35}$^*$ & \textbf{27.34}$^*$ \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{-0.4em}
  \footnotesize{Baselines from published papers. DA-TransUNet$^\dagger$ denotes our 300-epoch reproduction with best-model checkpoint selection, achieving 80.51\% which exceeds the published 79.80\%; the gain is due to best-model selection capturing validation peaks vs.\ final-epoch evaluation. Both DA-TransUNet$^\dagger$ and AdaDA use best-model selection consistently. $^*$Preliminary result from partial training; full 300-epoch result pending.}
\end{table}
...
\begin{table*}[t]
  ...
  $^\S$~AdaDA multi-GPU experiments use 2$\times$T4 with \texttt{CUDA\_VISIBLE\_DEVICES=0,1}, per-GPU batch size 12 (total=24, matching the single-GPU baseline) and base LR 0.01 unchanged—identical hardware to the baseline for a like-for-like comparison. AdaDA's windowed low-rank PAM reduces the attention matrix from $(B\!\times\!12544\!\times\!12544)$ to $(B\!\times\!256\!\times\!49\!\times\!49)$ at the $112\!\times\!112$ skip connection—a ${\sim}260\times$ reduction—enabling DataParallel training and approximately halving wall-clock time.}
\end{table*}
...
\begin{table*}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    DA-TransUNet (baseline)    & 80.51        & 25.41        & 25.5 & 11.5 GB & 121.8        & 0.5 GB       \\
