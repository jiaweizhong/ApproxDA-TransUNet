...
\begin{table*}[t]
  ...
  \begin{tabular}{lccccccc}
    \toprule
    Method & Params (M) & GFLOPs & Train Time & Train VRAM & Infer Time (ms/vol) & Infer VRAM & Multi-GPU \\
    \midrule
    TransUNet~\cite{chen2021transunet}     & 105.3       & 57.4        & --               & --               & --               & --               & No \\
    Swin-Unet~\cite{cao2022swinunet}       & 41.4        & 11.5        & --               & --               & --               & --               & No \\
    DA-TransUNet~\cite{sun2024datransunet} & \textit{XX} & \textit{XX} & 7.3h (T4$\times$1) & \textit{XX}   & \textit{XX}      & \textit{XX}      & No$^\ddagger$ \\
    \midrule
  ...
  \end{center}
  \vspace{-0.4em}
  \footnotesize{GFLOPs by \texttt{thop} on a single $224\!\times\!224$ slice. Train VRAM = peak during backward pass; Infer Time = avg ms per CT volume over 12 test cases; Infer VRAM = peak during forward pass. TransUNet/Swin-Unet GFLOPs from published papers. \textit{XX} = pending.\\
...
\begin{table*}[t]
  ...
  \begin{tabular}{lcccccc}
    \toprule
    Configuration & DSC (\%) $\uparrow$ & HD95 (mm) $\downarrow$ & GFLOPs & Train VRAM & Infer Time (ms/vol) & Infer VRAM \\
    \midrule
    DA-TransUNet (baseline)    & 79.36        & 26.64        & \textit{XX} & \textit{XX} & \textit{XX} & \textit{XX} \\
