...
\begin{figure}[ht]
  ...
  \caption{Gate value $g$ vs.\ per-block feature entropy $H(F)$ ($M{=}7$, gate=learn). Top-left: bottleneck; top-right: 28$^2$ skip; bottom-left: 56$^2$ skip; bottom-right: 112$^2$ skip. All four locations cluster at $g{\approx}0.5$ with $\Delta g{\leq}0.0002$.}
...
\textbf{Empirical evidence.} Quantitative gate monitoring confirms this collapse is near-total across all window configurations. After 300 epochs, the gate range at $M{=}7$ is $[0.4993,\,0.5010]$ with $\Delta g{=}0.0017$. At $M{=}14$: $g{\approx}0.5002$ with $\Delta g{\approx}0.0000$.
...
The ablation studies yield three converging insights: (1) spatial receptive field matters more than approximation rank---intermediate windows outperform both extremes while increasing rank yields no additional benefit; (2) task-specific GCS governs window sensitivity, with a 3.6$\times$ span ratio between Synapse and Kvasir-SEG confirming that high-GCS tasks require careful window tuning while low-GCS tasks are window-robust; and (3) scalar gating is insufficient for adaptive routing due to gradient symmetry, collapsing to a degenerate $g{\approx}0.5$ equilibrium. Together, these findings position attention approximation as a task-dependent inductive bias rather than a universal efficiency gain.

\subsection{Limitations}

GCS is currently an empirical characterization rather than a predictive metric: estimating task GCS without performing window-size ablations remains an open problem, and future work should validate the hypothesis on a broader range of datasets and develop theoretically grounded estimators. In addition, the proposed inductive-bias explanation remains an empirical hypothesis whose formal theoretical basis warrants further investigation.
