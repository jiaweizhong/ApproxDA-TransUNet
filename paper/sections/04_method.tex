...
\begin{figure*}[t]
  ...
  \label{fig:block}
\end{figure*}
% TODO(figure): redraw approxda_block.pdf for the c16 block (C/16 reduction,
% 3x3 conv blocks, per-channel gate of size C/16, no outer residual); see
% ../ApproxDA_ISBI_Reduction_and_Revision_Plan.md Sec. 4.1.
...
Windowing and low-rank projection reduce the PAM attention matrix from $N{\times}N$ to $N{\times}r$, a theoretical reduction of the attention-matrix term by $N/r \approx 390$ at the $112^2$ skip with $r{=}32$. This applies to the attention operation only; the measured cost of the attention blocks and of the whole network is reported in Section~\ref{sec:efficiency}.
