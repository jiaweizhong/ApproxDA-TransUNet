...
Figure~\ref{fig:framework} illustrates the overall ApproxDA-TransUNet architecture. Figure~\ref{fig:approxda_block} details the internal structure of each ApproxDABlock.
...
\begin{figure}[t]
  ...
  \end{tikzpicture}
  \caption{ApproxDA-TransUNet's three-dimensional approximation space. Each axis is independently controllable. Spatial scope ($M$) most directly governs long-range context capture and thereby sensitivity to window selection on high-CS tasks.}
...
This single-parameter design enables systematic ablation of routing strategy without any other architectural change. ApproxDA-TransUNet does not treat this routing module as a primary architectural contribution; instead, it serves as an analyzable component whose optimization behavior is investigated in Section~\ref{sec:understanding}. As observed in Section~\ref{sec:understanding}, the routing coefficient consistently converges to an almost constant value, revealing an unexpected optimization phenomenon.
