...
The original DA module performs dense spatial and channel attention over the full feature map, with quadratic complexity in both spatial resolution and channel dimension. ApproxDA-TransUNet reduces this cost along three independent axes (Figure~\ref{fig:approx_space}), each targeting a distinct aspect of the approximation:
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \draw[axis, blue!70!black]  (c) -- ++(0, 2.0)
      node[lab, above] {Spatial Scope\\{\scriptsize(window $M$)}};
    \draw[axis, red!60!black]   (c) -- ++(-2.0,-1.1)
      node[lab, below left] {Representation\\Fidelity\\{\scriptsize(rank $r$)}};
    \draw[axis, green!60!black] (c) -- ++(2.0,-1.1)
      node[lab, below right] {Semantic\\Interaction\\{\scriptsize(groups $G$)}};
  \end{tikzpicture}}
  \caption{The three orthogonal approximation axes of ApproxDA-TransUNet. Each axis is independently controllable, enabling clean ablations that isolate individual approximation effects.}
...
These axes correspond to three distinct types of approximation: \emph{spatial approximation} (how far attention reaches, controlled by $M$), \emph{representation approximation} (how precisely affinity is computed, controlled by $r$), and \emph{channel approximation} (how broadly channels interact, controlled by $G$). The axes are fully independent: any two of $\{M,\,r,\,G\}$ can be held fixed while the third varies, enabling clean ablations that isolate individual approximation effects. Figure~\ref{fig:approxda_block} shows the resulting block structure, with both branches fused through the configurable gate.

\begin{figure*}[tbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/approxda_block}
  \caption{ApproxDABlock internals. Input $\mathbf{x}$ splits into two branches: Low-Rank Windowed PAM (blue, upper) via window partition, low-rank projection, and attention; Grouped CAM (green, lower) via group split and channel attention. A learned gate $g$ fuses the outputs; $\mathbf{x}$ is added as a residual.}
