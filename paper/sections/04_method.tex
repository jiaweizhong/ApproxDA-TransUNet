...
\textbf{Low-rank projection.} Following~\cite{wang2020linformer}, query and key features inside each window are projected from $N_w {=} M^2$ to rank $r \ll M^2$ via a learned matrix $\mathbf{W}_r \in \mathbb{R}^{r \times N_w}$, where $\mathbf{C}, \mathbf{D} \in \mathbb{R}^{N_w \times C}$ denote the query and key feature matrices within each window:
\begin{equation}
  \tilde{\mathbf{C}} = \mathbf{W}_r \mathbf{C}, \quad \tilde{\mathbf{D}} = \mathbf{W}_r \mathbf{D} \in \mathbb{R}^{r \times C}.
  \label{eq:lowrank}
\end{equation}
...
\begin{equation}
  ...
  \label{eq:groupedcam}
\end{equation}
with total cost $\mathcal{O}(C^2N/G)$. Compared with dense interaction, grouped attention reduces computational complexity while preserving local channel dependencies.
