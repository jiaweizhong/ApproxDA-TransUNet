...
\begin{figure*}[tbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/approxda_overview}
...
These approximations reduce layer-level attention complexity: windowed PAM from $\mathcal{O}(N^2C)$ to $\mathcal{O}(NrC)$ ($r \ll N$); grouped CAM from $\mathcal{O}(C^2N)$ to $\mathcal{O}(C^2N/G)$.
