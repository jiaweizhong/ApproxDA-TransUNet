...
A valid causal factor must achieve strong positive rank correlation ($\rho > 0.8$) with the empirical GCS ranking across all five datasets (Synapse $>$ ACDC $>$ Kvasir $\approx$ CVC $>$ ISIC). Table~\ref{tab:causal} and Figures~\ref{fig:gcs_causal} and~\ref{fig:gcs_mask} present the elimination results.
...
\begin{figure*}[t]
  ...
  \caption{Ablation of causal candidate features across five medical segmentation benchmarks. (a) Context Sensitivity $\Delta\text{DSC}$ across the five evaluated tasks. (b)--(f) Candidates SC1--SC5 plotted against $\Delta\text{DSC}$; Spearman $\rho$ and Pearson $r$ annotated. ACDC (triangle) serves as a critical discriminating benchmark: it shares the multi-class nature of Synapse but exhibits minimal context sensitivity ($\Delta\text{DSC}=0.73$\,pp), falsifying SC1--SC4 and isolating SC5 (inter-class window separation) as the sole consistent causal predictor ($\rho = +0.89$, $r = +0.93$).}
  \label{fig:gcs_causal}
\end{figure*}

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/gcs_mask_sanity.pdf}
  \caption{Training-free spatial metrics (SC1--SC4) across five benchmarks. ACDC (coral) closely tracks binary low-GCS tasks across all four spatial-dispersion metrics, falsifying the hypothesis that context sensitivity is driven by target scale, boundary complexity, or multi-component crossing alone.}
  \label{fig:gcs_mask}
\end{figure*}
