...
if args.gate_mode != "learn":
    sys.exit(
        "Gate entropy analysis only applies to gate_mode='learn' (other modes have no learned gate)."
    )

# ---------- build snapshot path (mirrors train.py logic exactly) ----------
dataset_tag = {"Synapse": "Synapse224", "Kvasir": "Kvasir224", "ISIC": "ISIC224"}[
    args.dataset
]
...
approxda_blocks = [
    (name, m) for name, m in net.named_modules() if isinstance(m, ApproxDABlock)
]
...
def _infer_volume(image_vol, label_vol, num_fg_classes):
    ...
    for s in range(image_vol.shape[0]):
        ...
        if h != args.img_size or w != args.img_size:
            slice_img = nd_zoom(
                slice_img, (args.img_size / h, args.img_size / w), order=3
            )
...
if args.dataset == "Synapse":
    ...
    if not img_paths:
        sys.exit(
            "No images found under: " + dataset_root + "/images/  — run Synapse first."
        )
    print("Running inference on {} {} images …".format(len(img_paths), args.dataset))
    for ip, mp in zip(img_paths, msk_paths):
        img = (
            np.array(
                Image.open(ip).convert("L").resize((args.img_size, args.img_size))
            ).astype(np.float32)
            / 255.0
        )
        msk = (
            np.array(Image.open(mp).resize((args.img_size, args.img_size))) > 0
        ).astype(np.uint8)
...
if not active_idx:
    ...
    sys.exit("No hooks fired — check that best_model.pth loaded correctly.")
...
for i in active_idx:
    ...
    flag = (
        "PROCEED"
        if abs(r_H) > 0.3 and p_H < 0.01
        else ("WEAK" if abs(r_H) < 0.1 else "CHECK")
    )
    print(
        f"  {name:50s}  H: r={r_H:+.4f} p={p_H:.2e}  [{flag}]  |  Var: r={r_Var:+.4f} p={p_Var:.2e}"
    )

# ---------- Figure D stats: high vs low entropy group comparison ----------
print("\n=== Figure D: high-H vs low-H gate comparison (top/bottom 20%) ===")
figD_data = []
for i, (name, r_H, p_H, r_Var, p_Var, H_all, Var_all, g_all) in enumerate(
    spearman_results
):
    ...
    print(
        f"  Block {i:2d}  high-H mean_g={g_hi.mean():.4f}  "
...
if args.dataset == "Synapse":
    ...
    for cls_id, organ_name in SYNAPSE_ORGANS.items():
        ...
        cat_r.append((r, p))
        print(
            f"  {organ_name:15s}  n={mask.sum():4d}  "
    ...
    for qi in range(4):
        ...
        cat_r.append((r, p))
        print(
            f"  {raw_names[qi]:20s}  n={mask.sum():4d}  FG<={boundaries[qi+1]:.3f}  "
            f"mean_H={h_sub.mean():.4f}  mean_g={g_sub.mean():.4f}  "
            f"r_s={r:+.4f}  p={p:.2e}"
        )
...
try:
    ...
    rows = (n + cols - 1) // cols
    labels = [
        ".".join(name.split(".")[-2:]) if "." in name else name
    ...
    for i, (name, r_H, p_H, r_Var, p_Var, H_all, Var_all, g_all) in enumerate(
        ...
        sig = (
            "***"
            if p_H < 0.001
            else ("**" if p_H < 0.01 else ("*" if p_H < 0.05 else "n.s."))
        )
    ...
    for i, (name, r_H, p_H, r_Var, p_Var, H_all, Var_all, g_all) in enumerate(
        ...
        sig = (
            "***"
            if p_Var < 0.001
            else ("**" if p_Var < 0.01 else ("*" if p_Var < 0.05 else "n.s."))
        )
    ...
    if cat_names:
        ...
        ax.bar(
            x_cat - w / 2, H_norm, w, label="Normalised mean $H(F)$", color="steelblue"
        )
        ...
        ax.legend(fontsize=FS - 1)
        ax.set_title(
            "Per-class: feature entropy vs gate activation (block avg)", fontsize=FS
        )
        fig.tight_layout()
        ...
        fig.tight_layout()
        path_F = os.path.join(
            args.out_dir, f"fig_F_{label_suffix}_per_class_spearman.png"
        )
...
best_p_H = min(
    (p_H for _, r_H, p_H, *_ in spearman_results if abs(r_H) > 0.3), default=1.0
)
...
print(
    f"  Gate collapse check: max |delta| across blocks = {max_delta:.4f}  "
...
if gate_collapsed:
    ...
    print(
        "  Spearman r may look high but reflects rank ordering on negligible magnitude variation."
    )
    ...
    print(
        "  -> Accuracy gap is from windowed attention losing global context, not from gating."
    )
    ...
    print(
        "  -> Proceed to Phase 2. Narrative: explicit entropy strengthens implicit signal."
    )
elif max_r_H < 0.1:
    if max_r_Var > 0.2:
        print("Case 3 — H near zero, but Var correlates.")
        print(
            "  -> Implement Plan B1 (variance gate): gate_fc = Linear(C+1, C) with var input."
        )
    else:
        print("Case 3 — H near zero, Var also weak.")
        print(
            "  -> Implement Plan B3 (multi-statistic gate): gate_fc = Linear(C+2, C)."
        )
else:
...
if cat_names:
    ...
    for cname, (r, p) in zip(cat_names, cat_r):
        ...
        flag = (
            ...
            else ("weak" if abs(r) > 0.1 else "none")
        )
