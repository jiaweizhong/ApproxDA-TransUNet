...
def collect_gate_entropy(net, dataset_name):
    ...
    hooks = [
        blk.register_forward_hook(make_hook(i)) for i, (_, blk) in enumerate(blocks)
    ]
    ...
    def _fwd(img_2d):
        ...
        t = (
            torch.from_numpy(img_2d.astype(np.float32))
            .unsqueeze(0)
            .unsqueeze(0)
            .repeat(1, 3, 1, 1)
        )
    ...
    print(
        f"  Gate range [{g_avg.min():.4f}, {g_avg.max():.4f}]  "
...
try:
    ...
    for ext in ("pdf", "png"):
        fig.savefig(
            os.path.join(args.out_dir, f"fig_gate_collapse.{ext}"),
            dpi=DPI,
            bbox_inches="tight",
        )
    plt.close(fig)
    ...
    sig = (
        "***"
        if p_all < 0.001
        else ("**" if p_all < 0.01 else ("*" if p_all < 0.05 else "n.s."))
    )
    ...
    ax.set_title(
        ...
        r"$\Delta g_{\max}$="
        + f"{delta_all * G_SCALE:.2f}$\times 10^{{-3}}$ — gate is inert",
        fontsize=FS,
    )
    ...
    for ext in ("pdf", "png"):
        fig.savefig(
            os.path.join(args.out_dir, f"fig_entropy_scatter.{ext}"),
            dpi=DPI,
            bbox_inches="tight",
        )
    plt.close(fig)
