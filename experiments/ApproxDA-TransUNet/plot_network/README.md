...
- **R50 encoder**: Root (112×64) → Block1 (56×256) → Block2 (28×512) → Block3+PatchEmbed+ApproxDA (14×768)
...
### approxda_block.py
ApproxDABlock internals (two parallel branches + gate):
...
PlotNeuralNet is purely declarative — the Python scripts describe *what to draw*, not the actual computation graph. The layer sizes and connections in these scripts were derived by reading `Architecture/ApproxDATransUNet.py` and `Architecture/block.py` manually.
