...
def build_snapshot_path(args):
    ...
    return (
        ...
        else os.path.join(snap, f"epoch_{args.max_epochs - 1}.pth")
    )
...
def build_model(args, num_classes, window_size, gate_mode, snapshot_path):
    ...
    model = ApproxDATransUNet(
        config_vit, img_size=args.img_size, num_classes=num_classes
    ).cuda()
    ...
    if detected_gate != gate_mode:
        print(
            f"  [auto] gate_mode {gate_mode!r} -> {detected_gate!r} (from checkpoint)"
        )
...
class PAMEffectCollector:
    ...
    def aggregate(self, target_h=PATCH_SIZE, target_w=PATCH_SIZE):
        ...
        for name, eff in self.effects.items():
            ...
            up = F.interpolate(
                eff_4d, size=(target_h, target_w), mode="bilinear", align_corners=False
            )
...
def run_single_image(image_np, label_np, model, collector):
    ...
    with torch.no_grad():
        logits = model(inp)
        pred224 = (
            torch.argmax(torch.softmax(logits, dim=1), dim=1).squeeze(0).cpu().numpy()
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase E — PAM Attention Map Visualization"
    )
    parser.add_argument(
        "--dataset", type=str, default="Kvasir", choices=["Kvasir", "ISIC", "Synapse"]
    )
    ...
    parser.add_argument(
        "--snapshot", type=str, default="", help="Override checkpoint path"
    )
    ...
    parser.add_argument(
        "--seed_sample", type=int, default=42, help="Random seed for image selection"
    )
    parser.add_argument(
        "--select_samples",
        type=int,
    ...
    if args.compare_snapshot:
        ...
        model_b = build_model(
            args, num_classes, args.compare_window, "pam", args.compare_snapshot
        )
    ...
    # --- Load test data ---
    db_test = cfg["Dataset"](
        base_dir=cfg["volume_path"], split=cfg["split"], list_dir=cfg["list_dir"]
    )
    ...
    for i, batch in enumerate(loader):
        ...
        img_disp = (image_raw - image_raw.min()) / (
            image_raw.max() - image_raw.min() + 1e-8
        )
        ...
        if h != PATCH_SIZE or w != PATCH_SIZE:
            gt224 = scipy_zoom(
                label_raw.astype(float), (PATCH_SIZE / h, PATCH_SIZE / w), order=0
            )
        ...
        pred_a, attn_a, pred224_a, _ = run_single_image(
            img_disp, label_raw, model_a, collector_a
        )
        if h != PATCH_SIZE or w != PATCH_SIZE:
            pred224_a = scipy_zoom(
                pred_a.astype(float), (PATCH_SIZE / h, PATCH_SIZE / w), order=0
            )
        ...
        if model_b is not None:
            pred_b, attn_b, pred224_b, _ = run_single_image(
                img_disp, label_raw, model_b, collector_b
            )
            if h != PATCH_SIZE or w != PATCH_SIZE:
                pred224_b = scipy_zoom(
                    pred_b.astype(float), (PATCH_SIZE / h, PATCH_SIZE / w), order=0
                )
    ...
    if args.select_samples is not None:
        ...
        print(
            f"Plotting {len(plot_samples)}/{len(samples)} selected samples: {args.select_samples}"
        )
    else:
    ...
    n_cols = (
        4 if has_compare else 2
    )  # (img, attn_a) or (img, attn_a, attn_b_side_by_side)
    if has_compare:
    ...
    for row, s in enumerate(plot_samples):
        ...
        ax = axes[row, col]
        overlay_heatmap(
            ax, s["img224"], s["attn_a"], title=primary_label if row == 0 else ""
        )
        ...
        if has_compare:
            ax = axes[row, col]
            overlay_heatmap(
                ax, s["img224"], s["attn_b"], title=compare_label if row == 0 else ""
            )
    ...
    fig_suffix = (
        f'_sel{"_".join(str(i) for i in args.select_samples)}'
    ...
    if args.split_rows:
        ...
        for row in range(n_rows):
            ...
            bbox_inches = Bbox.union(tight_bboxes).transformed(
                fig.dpi_scale_trans.inverted()
            )
            fig.savefig(
                os.path.join(row_dir, f"row{row}.png"), dpi=200, bbox_inches=bbox_inches
            )
        plt.close(fig)
    ...
    with open(csv_path, "w") as f:
        ...
        for si, s in enumerate(samples):
            ...
            for model_tag, attn in [(primary_label, s["attn_a"])]:
                ...
                f.write(
                    f"{si},{model_tag},{mean_a:.4f},{max_a:.4f},{on_mask:.4f},{off_mask:.4f}\n"
                )
            if has_compare and "attn_b" in s:
                ...
                off_mask = float(attn[bg].mean()) if bg.any() else 0.0
                f.write(
                    f"{si},{compare_label},{mean_a:.4f},{max_a:.4f},{on_mask:.4f},{off_mask:.4f}\n"
                )
