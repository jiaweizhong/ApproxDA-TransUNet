...
def center_crop_and_resize(
    image_224: np.ndarray, crop_size: int, patch_size: int = 224
) -> np.ndarray:
    """
...
def infer_with_crop(
    image, label, model, num_classes: int, patch_size: int, crop_size: int
):
    """
    ...
    with torch.no_grad():
        if len(image.shape) == 3:
            ...
            if h != patch_size or w != patch_size:
                prediction = scipy_zoom(
                    prediction, (h / patch_size, w / patch_size), order=0
                )
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="Synapse", choices=["Synapse", "Kvasir", "ISIC"]
    )
    ...
    logging.info(
        f"Model loaded. Params: {sum(p.numel() for p in model.parameters())/1e6:.2f} M"
    )
    ...
    for cs in CONTEXT_SIZES:
        ...
        logging.info(
            f'  {cs:>12}  {pct:>7}%  {r["dsc"]:>8.4f}  {r["hd95"]:>8.2f}{marker}'
        )
    logging.info(sep)

    drop = results[224]["dsc"] - results[56]["dsc"]
    logging.info(
        f"  DSC drop (224->56): {drop:+.4f}  "
